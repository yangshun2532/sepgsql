...
SELECT relname, security_label FROM pg_class WHERE oid = 't1'::regclass;
SELECT attname, security_label FROM pg_attribute WHERE attrelid = 't1'::regclass and attnum > 0;

CREATE TABLE t2 (
       a int,
       b text
) SECURITY_LABEL = 'unconfined_u:object_r:sepgsql_ro_table_t:s0';
SELECT relname, security_label FROM pg_class WHERE oid = 't2'::regclass;
SELECT attname, security_label FROM pg_attribute WHERE attrelid = 't2'::regclass and attnum > 0;
...
SELECT relname, security_label FROM pg_class WHERE oid = 't3'::regclass;
SELECT attname, security_label FROM pg_attribute WHERE attrelid = 't3'::regclass and attnum > 0;
...
CREATE FUNCTION f1 (int) RETURNS int
       ...
       AS 'SELECT $1 * 2';
SELECT proname, security_label FROM pg_proc WHERE oid = 'f1'::regproc;
...
CREATE FUNCTION f2 (int) RETURNS int
       LANGUAGE 'sql'
       AS 'SELECT $1 + $1';
SELECT proname, security_label FROM pg_proc WHERE oid = 'f2'::regproc;
