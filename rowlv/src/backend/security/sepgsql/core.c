...
sepgsqlGetDatabaseLabel(void)
{
	...
	{
		static security_context_t	dblabel = NULL;

		if (!dblabel)
		{
			security_class_t	tclass
				= sepgsqlTransToExternalClass(SEPG_CLASS_DB_DATABASE);

			if (security_compute_create_raw(sepgsqlGetClientLabel(),
											sepgsqlGetClientLabel(),
											tclass, &dblabel) < 0)
				elog(ERROR, "SELinux: unable to compute a security context");
		}
		return pstrdup(dblabel);
	}
	...
	result = securityLookupSecurityLabel(dbsid);
	if (!result || security_check_context(result) < 0)
		result = pstrdup(sepgsqlGetUnlabeledLabel());
...
sepgsqlGetDatabaseSid(void)
{
	...
	if (IsBootstrapProcessingMode())
	{
		security_context_t dblabel
			= sepgsqlGetDatabaseLabel();

		dbsid = securityLookupSecurityId(dblabel);
	}
	else
	{
		HeapTuple	tuple;
