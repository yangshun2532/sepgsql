					...
					const char *audit_name, bool abort)
{
	...
	scontext = (security_check_context_raw(scontext) < 0
				? sepgsqlGetUnlabeledLabel() : scontext);
	tcontext = (security_check_context_raw(tcontext) < 0
				? sepgsqlGetUnlabeledLabel() : tcontext);
